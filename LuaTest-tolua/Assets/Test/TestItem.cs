...
public class TestLua : TestItem
{
    ...
    public override IEnumerator Test()
    {
        ...
        double totalMS = 0;
        LuaFunction f = m_ptest.state.GetFunction(m_luaFuncName);
        ...
        for (int i = 1; i <= count; ++i)
        {
            ...
            var rets = f.Invoke<Transform, float>(m_trans);
            double t = (double)rets * 1000;
            totalMS += t;
...
public class TestEmptyFunc : TestItem
{
    ...
    public override IEnumerator Test()
    {
        LuaFunction f = m_ptest.state.GetFunction("EmptyFunc");
        int count = m_ptest.runCount;
        double totalMS = 0;
        for (int i = 1; i <= count; ++i)
        {
...
public class TestGetLuaValue : TestItem
{
    ...
    public override IEnumerator Test()
    {
        ...
        for (int i = 1; i <= count; ++i)
        {
            ...
            for (int j = 0; j < 200000; ++j)
            {
                m_value = m_ptest.state[m_valueName];
            }
            double t = (double)((System.DateTime.Now.Ticks - ts) / 10000.0);
